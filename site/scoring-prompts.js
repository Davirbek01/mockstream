...
⚖️ CALIBRATION — score by LANGUAGE CONTROL + range + task fulfilment together, and apply the thresholds in BOTH directions:
• ERROR DENSITY drives the grammar/vocabulary judgement — weigh the number of errors against the length:
   – Frequent or systematic errors in SIMPLE structures (subject–verb agreement, plurals, articles, basic verb forms, word formation) that recur throughout → score the task 2 (A2) or 3 (Lower B1), EVEN IF it is well-organised and fully on-topic. Good organisation and task coverage do NOT lift a task above the level its grammar and vocabulary control actually support. As a guide, roughly one basic error every 1–2 sentences = a 2–3, not a 4.
   – Simple structures mostly accurate, with errors appearing mainly when attempting COMPLEX structures → 4.
   – Essentially error-free with good range and appropriate register → 5 (Part 2: 6 only if clearly C2).
• Do NOT UNDER-level strong work: a genuinely error-free, on-topic response with good range earns the TOP band — 5 for Task 1.1 ("above B1") and for Task 1.2 / Part 2 (C1). Do not reserve 5 for "exceptional", and do not default such work to 4. (A system's own top-band model answer must reach the top band.)
• Do NOT OVER-level weak work: a response with many basic errors is NOT B2 or C1 however well structured — frequent simple-structure errors cap it at 2–3. Off-topic / no real attempt → 0–1.
