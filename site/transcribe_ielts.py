"""
Transcribe IELTS Listening audio files using Gemini API.
...
def main():
    ...
    print(f"Found {len(files)} IELTS listening test files")
    print(f"Total audio parts to transcribe: {len(files) * 4}")

    for filename in files:
        filepath = os.path.join(QUESTIONS_DIR, filename)
        process_test_file(filepath)

    print("\n\nAll done! Transcripts have been saved to the JS files.")
