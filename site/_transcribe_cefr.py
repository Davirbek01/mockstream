"""
Transcribe CEFR Listening audio parts using Gemini API.
"""
    ...
    payload = {
        "contents": [{
            "parts": [
                ...
                {
                    "text": (
                        "Transcribe this CEFR listening audio exactly and completely. "
...
def process_test_file(filepath):
    ...
    for i in range(len(empty_matches) - 1, -1, -1):
        ...
        try:
            ...
            old_text = content[m.start():m.end()]
            key_prefix = old_text.split(":")[0]  # "transcript" or transcript
            replacement = f'{key_prefix}: `{safe_transcript}`'
...
def main():
    tests = sys.argv[1:] if len(sys.argv) > 1 else ["15"]
    for test_num in tests:
        filepath = os.path.join(QUESTIONS_DIR, f"cefr-listening-test-{test_num}.js")
