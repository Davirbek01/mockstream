...
-- No direct access: the panel writes through record_report_resend(), the
-- dashboard reads through the two dashboard functions. All are security definer.
...
-- The rows list gains the second appearance. The marker rides in
-- metadata->>''resend_of'' (the original timestamp) because this function
-- returns SETOF results and cannot grow a column of its own.
...
BEGIN
  ...
  RETURN QUERY EXECUTE format(
    'WITH src AS (
       SELECT r.id, r.created_at, r.student_name, r.center, r.exam_type, r.skill,
              r.score, r.level, r.caption, r.report_path, r.mock_number,
              r.metadata, r.device_info, r.user_email, r.device_id
         FROM public.results r
       UNION ALL
       SELECT r.id, x.resent_at, r.student_name, r.center, r.exam_type, r.skill,
              r.score, r.level, r.caption, r.report_path, r.mock_number,
              coalesce(r.metadata, ''{}''::jsonb)
                || jsonb_build_object(''resend_of'', r.created_at, ''resend_at'', x.resent_at),
              r.device_info, r.user_email, r.device_id
         FROM public.results r
         JOIN public.report_resends x ON x.result_id = r.id AND x.in_dashboard
     )
     SELECT *
       FROM src r
...
-- The stats read the same union, so the cards agree with the table below them.
...
as $function$
WITH src AS (
  SELECT r.center, r.exam_type, r.skill, r.student_name, r.score, r.created_at,
         r.device_info, r.mock_number, r.metadata
    FROM public.results r
  UNION ALL
  SELECT r.center, r.exam_type, r.skill, r.student_name, r.score, x.resent_at,
         r.device_info, r.mock_number, r.metadata
    FROM public.results r
    JOIN public.report_resends x ON x.result_id = r.id AND x.in_dashboard
),
parsed AS (
  ...
  WHERE (p_center    IS NULL OR r.center    = p_center)
    ...
    AND (
      p_date_from IS NULL
      OR (r.created_at + (p_tz_offset_min || ' minutes')::interval)::date >= p_date_from
    )
    AND (
      p_date_to IS NULL
      OR (r.created_at + (p_tz_offset_min || ' minutes')::interval)::date <= p_date_to
    )
    AND (
      p_type IS NULL
...
SELECT jsonb_build_object(
  ...
  'desktop',       COUNT(*) FILTER (
                     WHERE device_info->>'type' IS NOT NULL
                       AND device_info->>'type' NOT IN ('Mobile','Tablet')
                   )
)
FROM parsed;
$function$;
