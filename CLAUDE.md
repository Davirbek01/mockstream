...
`site/sw.js` uses a network-first strategy for HTML/JS and cache-first for icons. The cache name has a version suffix — currently `mockstream-v1042`. **Bump it any time you change the precached shell or want to force clients to drop stale assets** — otherwise users keep serving the old version from cache.
